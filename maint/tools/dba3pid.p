/* ------------------------------------------------------------------------- * 
 *                 dbactmon.p  -  Database Activity Monitor                  *
 * ------------------------------------------------------------------------- */
...
/*    QAD TXP Variables */
def var qpid as integer format ">>>>>>>9" no-undo.
def var qprog as char format "x(20)" no-undo.
def var qmenu as char format "x(12)" no-undo.
def var quser as char format "x(10)" no-undo.
def var qconn as int  format ">>>9" no-undo.
def var qinter  as char format "x(8)" no-undo.
def var qdate as date format "99/99/99" no-undo.
def var qtime as char format "x(5)" no-undo.
def var qpuser as char format "x(8)" no-undo.
def var qdev as char format "x(15)" no-undo.
...
DEFINE TEMP-TABLE ttConnection NO-UNDO
  ...
  FIELD DbAccess   LIKE DICTDB._UserIO._UserIO-DbAccess
  INDEX Idx-Main   IS UNIQUE
          ConnUsr
...
/*         --------========<<<<<<<< Frames >>>>>>>>========--------          */
DEFINE FRAME fMain
  "#" qconn FORMAT ">>>9" NO-LABEL
  "PID:" qpid FORMAT ">>>>>9"  NO-LABEL
  "ID:" quser NO-LABEL
  "MENU:" qmenu NO-LABEL  
  "PROG:" qprog NO-LABEL SKIP
  "START:" qdate NO-LABEL qtime NO-LABEL
  "MODE:" qinter NO-LABEL
  "OSID:" qpuser NO-LABEL
  "@" qdev NO-LABEL
  bTableStat AT ROW  3 COL 7
             HELP 'Scroll/Page Up/Down, SPACE=Refresh, F4=Exit' 
  bIndexStat AT ROW  3 COL 1
             HELP 'Scroll/Page Up/Down, SPACE=Refresh, F4=Exit' 
...
PROCEDURE pChooseConnection:
  ...
  &SCOPED-DEFINE OpenConnQuery OPEN QUERY qConn FOR EACH ttConnection      WHERE string(ttConnection.ConnPID) BEGINS fiFilterUser BY ttConnection.DBAccess DESC.
  ...
  DEFINE VARIABLE fiFilterUser AS CHARACTER FORMAT 'X(256)':U 
    LABEL 'Filter for PID' VIEW-AS FILL-IN SIZE 14 BY 1 NO-UNDO.
  ...
  DEFINE BROWSE bConn QUERY qConn DISPLAY
    ...
    ttConnection.DBAccess   /*ConnTime */
    WITH NO-BOX 14 DOWN.
...
&ENDIF
  ...
  DO:
    ...
    THEN DO:
      ASSIGN opConnectionUsr = ttConnection.ConnUsr.
      RUN qadmon (opConnectionUsr, FALSE) .
    END.
    APPLY 'GO' TO FRAME dgSelConn.
    RETURN.
  END.

  ON VALUE-CHANGED OF bConn IN FRAME dgSelConn
  DO:
    HIDE MESSAGE NO-PAUSE.
    IF AVAILABLE ttConnection 
    THEN DO:
      ASSIGN opConnectionUsr = ttConnection.ConnUsr.
      RUN qadmon (opConnectionUsr, TRUE) .
    END.
    RETURN.
  END.
  ...
  DO ON ERROR   UNDO SEL-CONN-BLOCK, LEAVE SEL-CONN-BLOCK
    ...
    {&OpenConnQuery}
    APPLY "VALUE-CHANGED" TO bConn IN FRAME dgSelConn.
...
/*  EMPTY TEMP-TABLE ttConnection.   */
  ...
  FOR EACH DICTDB._Connect  NO-LOCK
     ...
     FIND FIRST ttConnection 
            WHERE ttConnection.ConnUsr = DICTDB._Connect._Connect-Usr
            NO-LOCK NO-ERROR. 
     IF NOT AVAILABLE(ttConnection) THEN DO:                                 
        CREATE ttConnection.
        ASSIGN ttConnection.DbAccess = 0 .
     END.
     ...
     FOR FIRST DICTDB._UserIO where DICTDB._UserIO._UserIO-Usr = 
                        DICTDB._Connect._Connect-Usr 
                        NO-LOCK :
          ttConnection.DbAccess = DICTDB._UserIO._UserIO-DbAccess - 
                                  ttConnection.DbAccess.
/*          display ttConnection.DbAccess ttConnection.ConnUsr. */
     END.
...
PROCEDURE qadmon:
    ...
    def input parameter qqconn as integer.
    ...
    for last mon_mstr  where mon__qadi01 = qqconn
                                    no-lock by oid_mon_mstr:
      assign qmenu = mon_mstr.mon_selection
             qprog = mon_mstr.mon_program
             quser = mon_mstr.mon_userid
             qdate = mon_date_start
             qtime = string(mon_time_start,"HH:MM")
             qinter = mon_interface             
             qpuser = mon_prog_user       .
    end.
    for first ttConnection where ConnUsr = qqconn no-lock:
            assign qpid = ConnPID.
            assign qdev = ConnDevice.
    end.
    IF iplMessage NE TRUE THEN
    DISPLAY qconn qpid quser qmenu qprog 
            qdate qtime qinter qpuser qdev 
	    with frame fMain.
    HIDE MESSAGE NO-PAUSE.
    IF AVAIL(mon_mstr)
    THEN MESSAGE qdate qtime quser ":" qmenu qprog.
    ELSE MESSAGE "No QAD user data".
