...
/*{preDefs.i}*/
/*********START ADDITION*****************************************************/
&SCOPED-DEFINE USE_APPSRV FALSE
/*********END OF ADDITION****************************************************/
...
/*********START ADDITION*****************************************************/
DEFINE VARIABLE         lcLogDirectory    AS    CHARACTER      NO-UNDO.
/*********END OF ADDITION****************************************************/
...
/*********START ADDITION*****************************************************/
ASSIGN lcLogDirectory = RIGHT-TRIM(SESSION:TEMP-DIRECTORY,IF OPSYS = "WIN32" THEN "~\" ELSE "/").
/*********END OF ADDITION****************************************************/
  ...
  IF vAllThreads OR
    ...
    IF CAN-FIND(FIRST {&ReplDb}ReplTableXref WHERE
      ...
      IF NOT vReplRunning THEN DO:
       ...
       /* RDU */
       ASSIGN vLastAlarmDate = TODAY - 1.
       /* RDU */
       
       IF vLastAlarmDate < TODAY THEN DO:
          MESSAGE "Sending Mail" VIEW-AS ALERT-BOX.
...
**********WITH***************************************************************/
	     OUTPUT STREAM mstream TO VALUE(lcLogDirectory + "/" + "blatAlert.LOG") APPEND.
/*********END OF REPLACEMENT*************************************************/
...
/*********START REPLACEMENT***************************************************
  ...
  END.
**********WITH***************************************************************/
          vTmpDir       = RIGHT-TRIM(SESSION:TEMP-DIRECTORY, strDirMrk)
          vScriptDir    = vTmpDir
          strPro2SQLDir = PDBNAME(1)
          vLogDir       = lcLogDirectory
          vSendTo       = "roger.duijckaerts@hp.com"
          strPro2SQLDir = UPPER(ENTRY(NUM-ENTRIES(strPro2SQLDir,strDirMrk),
                                                    strPro2SQLDir,strDirMrk))
          .
/*********END OF REPLACEMENT*************************************************/
...
/*********START REPLACEMENT***************************************************
      OS-COMMAND silent value
                ( 'echo "' + vFilename + '" | ' +
                  ...
                  ' > /dev/null 2>&1' ).
**********WITH***************************************************************/
      OS-COMMAND SILENT VALUE(SUBSTITUTE('uuencode "&1" "&2" | ' +
                                         'mailx '                +
                                         '-m '                   +
                                         '-s "&3" '              +
                                         '-r "&4" '              +
